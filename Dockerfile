...
RUN apt-get update && \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends -o \
        ...
        python3-wheel python-rosdep python-rosinstall-generator python-wstool python-rosinstall

RUN SSL_CERT_FILE=/usr/lib/ssl/certs/ca-certificates.crt rosdep init && rosdep update
